...
return (
    <div className='mxa-w-7xl mx-auto p-6'>
        <h1 className="text-3xl font-bold mb-5">Admin Dashboard</h1>
        {productsLoading || ordersLoading ? (<p>Loading products...</p>) : productsError ? (
  ...
  {(orders && orders.length > 0) ? (
    orders.map((order) => ( 
      <tr key={order._id} className="border-b hover:bg-gray-50 cursor-pointer">
        ...
        <td className="py-3 px-4">{order.user?.name || order.user?.email || "N/A"}</td>
    ...
    <tr>
      <td colSpan={4} className="text-gray-500 p-6 text-center">
