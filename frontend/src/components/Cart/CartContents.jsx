...
const CartContents = ({cart, userId, guestId}) => {
    ...
    const handleAddtocart = (productId, delta, quantity, size, color) =>{
        ...
        if (newQuantity >= 1){
            dispatch(updateCartItemQuantity({productId, quantity : newQuantity,userId, guestId, size, color}));
        }
    }
    ...
    <div>
        ...
        <div key={index} className='flex items-start justify-between py-4 border-b'>
            <div className='flex items-start'>
                <img src={product.image} alt={product.image} className='w-20 h-24 object-cover mr-4 roudded'/>
                <div>
                    <h3>
                        {product.name}
                    </h3>
